...
const BuilderProject = () => {
    ...
    useEffect(() => {
        ...
        const searchoption = window.location.search.split("=")[1]
        const fetchData = async () => {
            const searchoption = router.query.key || "";
            const projectcategoryPathseg = router.asPath.split("?");
            const projectcategoryLastpath =
                projectcategoryPathseg[projectcategoryPathseg.length - 1];
            console.log(projectcategoryLastpath);

            const lastProjectPath = projectcategoryLastpath.split("=");
            console.log(lastProjectPath);
            ...
            let filteredBuilder = projectDetailsArray.filter((item) => {
                return item.Address.toLowerCase().includes(searchoption.toLowerCase()) || item.project_name.toLowerCase().includes(searchoption.toLowerCase()) || item.builder_details.builder_name.toLowerCase().includes(searchoption.toLowerCase());
            });
            console.log(filteredBuilder);
            setProjectList(filteredBuilder);
    ...
    function formatString(inputString) {
        ...
        const capitalizedString =onChange={keyQuery}
